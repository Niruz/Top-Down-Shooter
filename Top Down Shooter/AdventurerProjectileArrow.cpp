...
#include "World.h"
#include "Level.h"
AdventurerProjectileArrow::AdventurerProjectileArrow(int id, const std::string& name, const glm::vec3& myStartPosition, const glm::vec3& direction)
	:BaseProjectileEntity(id, name, myStartPosition, direction)
{
	...
	myAnimatedSprite = new AdventurerProjectileSprite(glm::vec4(mPosition.x, mPosition.y, mPosition.z + 0.2f, 1), glm::vec2(34, 9), TextureMan->GetTexture("arrowProjectile"), Heading::RIGHTFACING);
...
bool AdventurerProjectileArrow::HandleMessage(const Message& msg)
{
	return false;
}