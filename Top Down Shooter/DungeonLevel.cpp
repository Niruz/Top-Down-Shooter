...
void DungeonLevel::Initialize()
{
	...
	myShader = ShaderMan->getShader(SIMPLE_DUNGEON_SHADER);

	myTileLayer = new Layer(new BatchRenderer(), myShader, glm::ortho(-320.0f - 64, 320.0f + 64, -180.0f - 36, 180.0f + 36, -100.0f, 100.0f));
	myDungeonBackground = new Sprite(glm::vec4(0, -25, -99.0f, 1), glm::vec2(768.0f, 432.0f-144-48), TextureMan->GetTexture("dungeonbackground"));
	myTileLayer->Add(myDungeonBackground);
	myPillarBackground = new Sprite(glm::vec4(0, -25, -98.0f, 1), glm::vec2(768.0f, 432.0f-144-48), TextureMan->GetTexture("pillarbackground"));
	myTileLayer->Add(myPillarBackground);
...
void DungeonLevel::UpdatePlayer()
{
	EntityMan->Update();
	CollisionMan->Update();
...
void DungeonLevel::Tick(const unsigned int ups, const unsigned int fps)
{
...
void DungeonLevel::DeInitialize()
{

}
void DungeonLevel::ProcessKeyBoard(int key, float deltaTime, int action)
{
	myPlayer->processKeyBoard(key, deltaTime, action);
}
void DungeonLevel::ProcessMouse(double xpos, double ypos, bool movement)
{
...
void DungeonLevel::AddEntity(Entity* entity)
{
	myEntitites.push_back(entity);
}

void DungeonLevel::SpawnEntity(const std::string& type, const glm::vec3&  inpos, const glm::vec3& indir, int heading)
{

}
void DungeonLevel::RemoveEntity(Entity* entity)
{
