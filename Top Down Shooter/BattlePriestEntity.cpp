...
BattlePriestEntity::BattlePriestEntity(int id, const std::string& name, const glm::vec3& myStartPosition, const glm::vec3& patrolTo)
	: BaseEnemy(id, name, myStartPosition, patrolTo, false)
{
	...
	myAnimatedSprite = new BattlePriestSprite(glm::vec4(mPosition.x, mPosition.y, mPosition.z, 1), glm::vec2(200, 200), TextureMan->GetTexture("battlepriest"), Heading::RIGHTFACING);
	mySprite->Add(myAnimatedSprite);
//	mySprite->Add(myPlayerAABB);
	myAnimatedSprite->SetAnimation("BattlePriestIdle");
	myXDirection = 1.0f;
	...
	myStateMachine = new StateMachine<BattlePriestEntity>(this);

	myStateMachine->setCurrentState(BattlePriestIdle::Instance());
	myStateMachine->changeState(BattlePriestIdle::Instance());
	...
	myHitAABB = new AABB(glm::vec2(mPosition.x - 50.0f, mPosition.y - 10.0f), 50.0f, 40.0f);
	myHitSpriteAABB = new Sprite(glm::vec4(mPosition.x - 50.0f, mPosition.y - 10.0f, mPosition.z + 0.01, 1.0f), glm::vec2(100.0f, 80.0f), glm::vec4(0.0f, 1.0f, 0.0f, 0.5f));
	//	mySprite->Add(myHitSpriteAABB);

	myAlreadyAttacked = false;
	myAlreadyAttacked2 = false;
	myHealth = 60;

}
BattlePriestEntity::~BattlePriestEntity()
{
	delete myStateMachine;
}
void BattlePriestEntity::Update()
{
...
bool BattlePriestEntity::HandleMessage(const Message& msg)
{
	myStateMachine->HandleMessage(msg);
	return false;
}
void BattlePriestEntity::SetFacing()
{
	...
	{
		myAABB->myOrigin = glm::vec2(mPosition.x + 10.0f, mPosition.y - 50.0f);
		myPlayerAABB->myPosition = glm::vec4(mPosition.x + 10.0f, mPosition.y - 50.0f, mPosition.z + 0.01, 1.0f);
		myHitAABB->myOrigin = glm::vec2(mPosition.x + 50.0f, mPosition.y - 50.0f);
		myHitSpriteAABB->myPosition.x = mPosition.x + 50.0f;
		myHitSpriteAABB->myPosition.y = mPosition.y - 50.0f;
	}
	else
	{
		myAABB->myOrigin = glm::vec2(mPosition.x - 10.0f, mPosition.y - 50.0f);
...
void BattlePriestEntity::HandleMovement()
{
	if (GetFSM()->isInState(*BattlePriestPatrol::Instance()))
	{
...
void BattlePriestEntity::SetAnimation(const std::string& name)
{
	myAnimatedSprite->SetAnimation(name);
	myAnimatedSprite->Reset();
}
void BattlePriestEntity::ResetAttackTimer()
{
	myAttackTimer = Clock->GetCurrentTimeInSeconds();
}
bool BattlePriestEntity::IsPlayerWithinPatrolRange()
{
...
bool BattlePriestEntity::IsPlayerWithinAttackDistance()
{
	//glm::vec2 playerPos = myTileMap->lastPlayerTile->myWorldPosition;
...
void BattlePriestEntity::HandleDamaged(int damageRecieved)
{
	...
	if (myHealth <= 0)
		GetFSM()->changeState(BattlePriestDie::Instance());
}